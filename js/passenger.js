import { auth, db, initFirebaseMessaging, listenForegroundMessages } from "./firebase.js";
...
const passengerAreaHint = $("#passengerAreaHint");
...
function closePassengerDrawer() {
  ...
  setTimeout(() => drawerBackdropPassenger?.classList.add("hidden"), 280);
}
...
function showSearchResults(container) { container?.classList.remove("hidden"); }
function setupSearchResultsAutoHide(inputEl, resultsEl) {
  if (!inputEl || !resultsEl) return;
  inputEl.addEventListener("input", () => {
    const hasText = (inputEl.value || "").trim().length > 0;
    if (!hasText) return clearSearchResults(resultsEl);
...
function resetActionVisibility() {
  showEl(btnAcceptOffer); showEl(btnRejectOffer); showEl(btnCancel); showEl(btnComplete);
  showEl(btnTrack); showEl(btnCall); showEl(btnWhats); showEl(btnChatPassenger);
}
...
function setDriverContactButtons(driverPhone) {
  ...
  if (!ph) {
    btnCall.disabled = true; btnWhats.disabled = true; btnCall.onclick = null; btnWhats.onclick = null; return;
  }
  btnCall.disabled = false; btnWhats.disabled = false;
...
function showRatingModal() { rateModal?.classList.add("show"); }
function hideRatingModal() { rateModal?.classList.remove("show"); }
...
function buildPricingSummary(distanceMeters, durationSec, manualPrice) {
  ...
  const baseFare = 15, perKm = 8, perMin = 0.35;
  const estimated = Math.round((baseFare + km * perKm + mins * perMin) * surge);
  return { baseFare, perKm, perMin, surge, distanceKm: Number(km.toFixed(2)), durationMin: Number(mins.toFixed(1)), estimatedPrice: estimated, finalPrice: clampPrice(manualPrice) };
}
...
function openPassengerChatModal() { chatModalPassenger?.classList.remove("hidden"); chatBackdropPassenger?.classList.add("show"); }
function closePassengerChatModal() { chatBackdropPassenger?.classList.remove("show"); chatModalPassenger?.classList.add("hidden"); }
function openPassengerTripsModal() { passengerTripsModal?.classList.remove("hidden"); passengerTripsBackdrop?.classList.add("show"); }
function closePassengerTripsModal() { passengerTripsBackdrop?.classList.remove("show"); passengerTripsModal?.classList.add("hidden"); }
...
function closePassengerWalletModal() { passengerWalletBackdrop?.classList.remove("show"); passengerWalletModal?.classList.add("hidden"); }
    ...
    passengerAccountInfo.innerHTML = `
      <div class="profile-card">
        ...
        <div class="profile-list">
          <div class="profile-item">📞 ${escapeHtml(myData?.phone || "—")}</div>
          <div class="profile-item">📍 ${escapeHtml(myData?.governorate || "—")} / ${escapeHtml(myData?.center || "—")}</div>
          <div class="profile-item">🚗 ${escapeHtml(myData?.vehicleType || "—")}</div>
        </div>
      </div>`;
  }
...
function closePassengerAccountModal() { passengerAccountBackdrop?.classList.remove("show"); passengerAccountModal?.classList.add("hidden"); }
...
function renderPassengerChatMessages(rows = []) {
  ...
  rows.forEach((msg) => {
    ...
    item.innerHTML = `<div class="chat-text">${escapeHtml(msg.text || "")}</div><div class="chat-meta"><span class="chat-name">${escapeHtml(msg.senderName || "")}</span><span class="chat-time">${time} ${mine ? (msg.read ? "✔✔" : "✔") : ""}</span></div>`;
...
async function sendPassengerChatMessage() {
  ...
  try {
    await addDoc(collection(db, "rides", currentRideId, "messages"), {
      text, senderId: myData.uid, senderRole: "passenger", senderName: myData.name || "الراكب", createdAt: serverTimestamp(), typing: false, read: false
    });
...
function clearRideSearchTimer() { if (rideSearchTimer) { clearInterval(rideSearchTimer); rideSearchTimer = null; } }
function formatSearchElapsed(seconds) { const s = Math.max(0, Number(seconds || 0)); return `${String(Math.floor(s / 60)).padStart(2, "0")}:${String(s % 60).padStart(2, "0")}`; }
function rideCreatedAtMs(ride) { return Number(ride?.createdAtMs || ride?.clientCreatedAtMs || ride?.createdAt?.toMillis?.() || Date.now()); }
...
async function findNearestDriversMeta({ governorate, center, vehicleType, pickupPoint, limit = 8 }) {
  try {
    const snap = await getDocs(query(collection(db, "driversOnline"), where("governorate", "==", governorate), where("center", "==", center)));
    const rows = snap.docs.map((d) => ({ uid: d.id, ...d.data() }))
      .filter((d) => canUseDriverOnlineRow(d))
      .filter((d) => !vehicleType || !d.vehicleType || d.vehicleType === vehicleType)
      .map((d) => ({ uid: d.uid, distanceToPickupM: Math.round(haversineMeters({ lat: d.lat, lon: d.lon }, pickupPoint)), vehicleType: d.vehicleType || null, lastSeenMs: Number(d.lastSeenMs || 0) }))
    ...
    return { nearestDriverId: rows[0]?.uid || null, nearestDriverIds: rows.map((d) => d.uid), nearestDrivers: rows };
  } catch (e) {
...
function stopDriverTracking() { if (driverTrackUnsub) driverTrackUnsub(); driverTrackUnsub = null; removeRouteLayer(driverRouteLayerRef); clearDriverMarker(); }

function clearAll() {
  pickup = null; dropoff = null; pickupText.value = ""; dropText.value = "";
  if (pickupMarker) try { map.removeLayer(pickupMarker); } catch (_) {}
  if (dropMarker) try { map.removeLayer(dropMarker); } catch (_) {}
  pickupMarker = null; dropMarker = null; removeRouteLayer(routeLayerRef);
  lastDistanceMeters = null; lastDurationSec = null;
  setText(priceValue, "—"); setText(distanceValue, "—"); setText(routeMeta, "اختر قيام/وصول لرسم المسار");
}

function stopLiveDrivers() {
  if (liveDriversUnsub) liveDriversUnsub();
  liveDriversUnsub = null;
  driverMarkers.forEach((obj) => { try { obj.marker.remove(); } catch (_) {} });
  driverMarkers.clear();
}
...
function hardResetPassengerUI() {
  ...
  removeRouteLayer(routeLayerRef); removeRouteLayer(driverRouteLayerRef);
  pickup = null; dropoff = null; lastDistanceMeters = null; lastDurationSec = null;
  if (pickupText) pickupText.value = ""; if (dropText) dropText.value = "";
  setDriverContactButtons(null);
  rideCard.innerHTML = `<div class="muted">لا يوجد طلب نشط.</div>`;
  setText(routeMeta, "اختر قيام/وصول لرسم المسار"); setText(distanceValue, "—"); setText(priceValue, "—"); setStatus("جاهز"); ratingValue = 0; if (rateComment) rateComment.value = ""; setText(rateHint, "");
...
async function reverseNameEG(lat, lon) {
  try {
    ...
    url.searchParams.set("format", "json"); url.searchParams.set("lat", String(lat)); url.searchParams.set("lon", String(lon)); url.searchParams.set("zoom", "18"); url.searchParams.set("addressdetails", "1");
    const res = await fetch(url.toString(), { headers: { "Accept-Language": "ar" } });
    if (!res.ok) return null;
    const data = await res.json(); const a = data?.address || {};
    const parts = [a.road || a.pedestrian || a.neighbourhood || a.suburb || a.city_district || "", a.city || a.town || a.village || a.county || "", a.state || ""].filter(Boolean);
    return parts.join("، ") || data?.display_name || null;
  } catch (_) { return null; }
}
function setPickup(point) { pickup = point; pickupText.value = point.text || point.display || ""; clearSearchResults(pickupResults); if (pickupMarker) try { map.removeLayer(pickupMarker); } catch (_) {} pickupMarker = addMarker(map, [point.lat, point.lon], { icon: createPickupIcon() }); updateRouteIfReady(); }
function setDropoff(point) { dropoff = point; dropText.value = point.text || point.display || ""; clearSearchResults(dropResults); if (dropMarker) try { map.removeLayer(dropMarker); } catch (_) {} dropMarker = addMarker(map, [point.lat, point.lon], { icon: createDropoffIcon() }); updateRouteIfReady(); }

async function manualSearch(type) {
  const isPickup = type === "pickup";
  const inputEl = isPickup ? pickupText : dropText;
  const q = (inputEl.value || "").trim();
  if (!q) return alert("اكتب اسم المكان");
  if (!myLocation?.lat || !myLocation?.lon) return notify({ title: "الموقع", body: "حدد موقعك أولاً (زر 🎯)" });
  try {
    clearSearchResults(isPickup ? pickupResults : dropResults);
    const items = await geocodeNominatim(q, 8, myLocation);
    const it = items?.[0]; if (!it) return alert("المكان غير موجود");
    const obj = { lat: Number(it.lat), lon: Number(it.lon), text: it.text || q };
    if (isPickup) setPickup(obj); else setDropoff(obj);
    map.setView([obj.lat, obj.lon], Math.max(map.getZoom(), 15));
  } catch (e) { console.error("SEARCH ERROR:", e); alert("خطأ في البحث"); }
}
...
async function updateRouteIfReady() {
  ...
  try {
    ...
    lastDistanceMeters = r.distanceMeters; lastDurationSec = r.durationSec; drawRoute(map, r.geojson, routeLayerRef);
    const km = (r.distanceMeters / 1000); const mins = Math.max(1, Math.round(r.durationSec / 60));
    ...
    setStatus("جاهز"); updatePriceUI();
  } catch (e) { console.error("ROUTE ERROR:", e); setStatus("خطأ"); setText(routeMeta, "تعذر رسم المسار. جرّب نقطتين مختلفتين."); }
}

function updateRideActionVisibility(ride) {
  resetActionVisibility();
  if (!ride) return rideUiNone();
  if (ride.status === "requested") { hideEl(btnAcceptOffer); hideEl(btnRejectOffer); hideEl(btnTrack); hideEl(btnComplete); hideEl(btnCall); hideEl(btnWhats); hideEl(btnChatPassenger); }
  else if (ride.status === "offered") { hideEl(btnTrack); hideEl(btnComplete); hideEl(btnCall); hideEl(btnWhats); hideEl(btnChatPassenger); }
  else if (ride.status === "accepted" || ride.status === "arrived" || ride.status === "started") { hideEl(btnAcceptOffer); hideEl(btnRejectOffer); }
  else if (ride.status === "completed" || ride.status === "canceled") { hideEl(btnAcceptOffer); hideEl(btnRejectOffer); hideEl(btnCancel); hideEl(btnComplete); hideEl(btnTrack); hideEl(btnCall); hideEl(btnWhats); hideEl(btnChatPassenger); }
}
...
function renderRideCard(ride, driverProfile) {
  ...
  if (ride.status === "requested") return renderSearchingRideCard(ride);
  const lines = [];
  ...
  if (["accepted", "arrived", "started", "completed"].includes(ride.status)) {
    lines.push(`<div class="divider"></div><div><b>السائق</b></div>`);
    ...
    if (driverProfile?.vehicleCode || ride.driverVehicleCode) lines.push(`<div class="muted small">كود المركبة: ${escapeHtml(driverProfile?.vehicleCode || ride.driverVehicleCode || "-")}</div>`);
  } else {
...
function startLiveDriversLayer({ governorate, center }) {
  ...
  liveDriversUnsub = onSnapshot(q, (snap) => {
    const seen = new Set();
    snap.forEach((row) => {
      const d = row.data(); const uid = row.id; const lat = Number(d.lat); const lon = Number(d.lon ?? d.lng); const lastSeenMs = Number(d.lastSeenMs || 0);
      if (![lat, lon].every(Number.isFinite) || Date.now() - lastSeenMs > DRIVER_ONLINE_MAX_AGE_MS) return;
      seen.add(uid);
    ...
    for (const [uid, obj] of driverMarkers.entries()) if (!seen.has(uid)) { try { obj.marker.remove(); } catch (_) {} driverMarkers.delete(uid); }
  }, (err) => console.error("PASSENGER liveDrivers ERROR:", err));
}
  ...
  currentRideListUnsub = onSnapshot(query(collection(db, "rides"), where("passengerId", "==", userId)), (snap) => {
    ...
    const docs = snap.docs.map((d) => ({ id: d.id, ...d.data() }))
      .filter((r) => {
        ...
        return r.passengerId === userId && activeOrFinished && (allowArchivedFinished || r.archived !== true) && (allowArchivedFinished || !isRideExpired(r, getRideFreshMaxAgeMs(r.status) || ACTIVE_RIDE_MAX_AGE_MS, now));
      })
      .sort((a, b) => (b.createdAt?.toMillis?.() || 0) - (a.createdAt?.toMillis?.() || 0));
    renderPassengerHistory(docs);
    const ride = docs[0] || null;
    if (!ride) return hardResetPassengerUI();
    if (currentRideId === ride.id && currentRideDocUnsub) return;
    if (currentRideDocUnsub) { currentRideDocUnsub(); currentRideDocUnsub = null; }
    currentRideId = ride.id;
    currentRideDocUnsub = onSnapshot(doc(db, "rides", currentRideId), (rideSnap) => {
      if (token !== rideWatcherToken) return;
      handleRideSnapshot(rideSnap);
    });
  }, (err) => { console.error("watchCurrentRide error:", err); hardResetPassengerUI(); });
}
...
bindSearch(pickupText, pickupResults, (it) => setPickup({ lat: Number(it.lat), lon: Number(it.lon), text: it.display || it.text || "" }), { getBiasLocation: () => myLocation });
bindSearch(dropText, dropResults, (it) => setDropoff({ lat: Number(it.lat), lon: Number(it.lon), text: it.display || it.text || "" }), { getBiasLocation: () => myLocation });
setupSearchResultsAutoHide(pickupText, pickupResults); setupSearchResultsAutoHide(dropText, dropResults);
...
menuBtnPassenger?.addEventListener("click", openPassengerDrawer); drawerClosePassenger?.addEventListener("click", closePassengerDrawer); drawerBackdropPassenger?.addEventListener("click", closePassengerDrawer);
priceSlider.addEventListener("input", () => { priceSlider.dataset.touched = "1"; updatePriceUI(); });
btnClear.addEventListener("click", clearAll);
btnLocate.addEventListener("click", () => locateOnce(map, (loc) => { myLocation = loc; showMyLocation(map, loc, { pan: true }); }));
pickupMyLoc?.addEventListener("click", async () => { if (!myLocation?.lat || !myLocation?.lon) return notify({ title: "الموقع", body: "حدد موقعك أولاً (زر 🎯)" }); const lat = Number(myLocation.lat), lon = Number(myLocation.lon); const name = (await reverseNameEG(lat, lon)) || "موقعي الحالي"; setPickup({ lat, lon, text: name }); map.setView([lat, lon], 16); });
pickupPick.addEventListener("click", () => { pickMode = "pickup"; setStatus("اختر القيام من الخريطة"); });
dropPick.addEventListener("click", () => { pickMode = "dropoff"; setStatus("اختر الوصول من الخريطة"); });
map.on("click", async (e) => { if (!pickMode) return; const lat = e.latlng.lat, lon = e.latlng.lng; const label = (await reverseNameEG(lat, lon)) || `(${lat.toFixed(5)}, ${lon.toFixed(5)})`; const point = { lat, lon, text: label }; if (pickMode === "pickup") setPickup(point); else setDropoff(point); pickMode = null; });
drawerAccountPassenger?.addEventListener("click", () => { closePassengerDrawer(); openPassengerAccountModal(); });
drawerTripsPassenger?.addEventListener("click", () => { closePassengerDrawer(); openPassengerTripsModal(); });
drawerWalletPassenger?.addEventListener("click", () => { closePassengerDrawer(); openPassengerWalletModal(); });
...
btnAcceptOffer.addEventListener("click", async () => {
  ...
  try {
    const rideRef = doc(db, "rides", currentRideId); const rideSnap = await getDoc(rideRef); if (!rideSnap.exists()) return;
    const ride = rideSnap.data(); if (ride.status !== "offered") return;
    await updateDoc(rideRef, { status: "accepted", price: ride.offerPrice, acceptedAt: serverTimestamp() });
    if (ride.pickup?.lat && ride.pickup?.lon) currentPickup = { lat: Number(ride.pickup.lat), lon: Number(ride.pickup.lon) };
    if (ride.driverId) startDriverTracking(ride.driverId);
    notify({ title: "تم قبول عرض السائق", body: `السعر النهائي: ${Math.round(ride.offerPrice)} ج`, tag: "offer-accepted" });
  } catch (e) { console.error(e); setStatus("خطأ"); }
});
...
btnTrack.addEventListener("click", () => { if (driverMarker) try { map.panTo(driverMarker.getLatLng()); } catch (_) {} });
btnComplete.addEventListener("click", async () => {
  if (!currentRideId) return; setStatus("يرسل طلب إنهاء...");
  try {
    await updateDoc(doc(db, "rides", currentRideId), { passengerEndRequested: true, passengerEndRequestedAt: serverTimestamp() });
    notify({ title: "تم", body: "تم إرسال طلب إنهاء الرحلة للسائق.", tag: "end-requested" }); setStatus("تم إرسال طلب إنهاء ✅");
  } catch (e) { console.error(e); setStatus("خطأ"); }
});
if (btnChatPassenger) btnChatPassenger.addEventListener("click", () => { if (!currentRideId) return; openPassengerChatModal(); watchPassengerChat(currentRideId); });
chatClosePassenger?.addEventListener("click", closePassengerChatModal); chatBackdropPassenger?.addEventListener("click", closePassengerChatModal);
passengerTripsClose?.addEventListener("click", closePassengerTripsModal); passengerTripsBackdrop?.addEventListener("click", closePassengerTripsModal);
passengerWalletClose?.addEventListener("click", closePassengerWalletModal); passengerWalletBackdrop?.addEventListener("click", closePassengerWalletModal);
passengerAccountClose?.addEventListener("click", closePassengerAccountModal); passengerAccountBackdrop?.addEventListener("click", closePassengerAccountModal);
chatSendPassenger?.addEventListener("click", sendPassengerChatMessage);
if (chatInputPassenger) {
  chatInputPassenger.addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); sendPassengerChatMessage(); } });
  chatInputPassenger.addEventListener("input", async () => { if (!currentRideId) return; try { await updateDoc(doc(db, "rides", currentRideId), { passengerTyping: true }); } catch (_) {} });
}
rateSend?.addEventListener("click", async () => {
  if (!currentRideId) return;
  if (!ratingValue) return setText(rateHint, "اختر عدد نجوم أولاً.");
  setText(rateHint, "جارٍ الإرسال...");
  try {
    const rideRef = doc(db, "rides", currentRideId); const rideSnap = await getDoc(rideRef); const ride = rideSnap.exists() ? rideSnap.data() : null;
    await updateDoc(rideRef, { passengerRating: ratingValue, passengerComment: (rateComment?.value || "").trim(), ratedAt: serverTimestamp(), archived: true });
    if (ride?.driverId) {
      const driverRef = doc(db, "users", ride.driverId); const driverSnap = await getDoc(driverRef);
      if (driverSnap.exists()) {
        const d = driverSnap.data(); const prevCount = Number(d.ratingCount || 0); const prevAvg = Number(d.ratingAvg || 0); const nextCount = prevCount + 1; const nextAvg = ((prevAvg * prevCount) + ratingValue) / nextCount;
        await updateDoc(driverRef, { ratingCount: nextCount, ratingAvg: Number(nextAvg.toFixed(2)), updatedAt: serverTimestamp() }).catch(() => {});
      }
    }
    hideRatingModal(); if (currentRideDocUnsub) { currentRideDocUnsub(); currentRideDocUnsub = null; } cleanupRideState(); rideUiNone(); notify({ title: "تم إرسال التقييم", body: "شكراً لمشاركتك رأيك.", tag: "rated" });
  } catch (e) { console.error("RATE ERROR:", e); setText(rateHint, "تعذر إرسال التقييم. جرّب مرة أخرى."); }
});
rateClose?.addEventListener("click", () => { hideRatingModal(); finalizePassengerRideCleanup(); });
rateSkip?.addEventListener("click", () => { hideRatingModal(); finalizePassengerRideCleanup(); });
rateModal?.addEventListener("click", (e) => { if (e.target === rateModal) { hideRatingModal(); finalizePassengerRideCleanup(); } });
starsRoot?.addEventListener("click", (e) => { const t = e.target; if (!t || !t.classList.contains("star")) return; renderStars(Number(t.dataset.v || 0)); });
...
function closeSwitchDriverModal() { $("#switchDriverBackdrop")?.classList.remove("show"); $("#switchDriverModal")?.classList.add("hidden"); }
$("#switchDriverClose")?.addEventListener("click", closeSwitchDriverModal); $("#switchDriverCancel")?.addEventListener("click", closeSwitchDriverModal); $("#switchDriverBackdrop")?.addEventListener("click", closeSwitchDriverModal);
$("#switchDriverSave")?.addEventListener("click", async () => {
  const hint = $("#switchDriverHint"); hint.textContent = "جارٍ التحويل...";
  const gov = $("#sdGov").value, center = $("#sdCenter").value, vehicleType = sdSelectedVehicle, vehicleCode = ($("#sdVehicleCode").value || "").trim(), address = ($("#sdAddress").value || "").trim();
  if (!gov || !center) return hint.textContent = "اختر المحافظة والمركز.";
  if (!vehicleType) return hint.textContent = "اختر نوع المركبة.";
  if (!vehicleCode) return hint.textContent = "اكتب كود المركبة.";
  try {
    const u = auth.currentUser; if (!u) throw new Error("لا يوجد مستخدم مسجل.");
    await updateDoc(doc(db, "users", u.uid), { uid: u.uid, role: "driver", governorate: gov, center, vehicleType, vehicleCode, address, updatedAt: serverTimestamp() });
    location.href = "./driver.html?ts=" + Date.now();
  } catch (e) { console.error("SWITCH DRIVER ERROR:", e); hint.textContent = e?.message || "حدث خطأ أثناء التحويل"; }
});
